...
def parse_desktop_startup(path: Path, expected_desktop_hashes: set[str]) -> dict:
    ...
    status = PASS if declared_pass and alive and cleanup and not failures and hash_bound else FAIL
    return {
        "status": status,
        "reportSha256": report_hash,
        "desktopSha256": desktop_hash,
        "desktopHashBoundToPublishedCandidate": hash_bound,
        "aliveWindowSeconds": payload.get("aliveWindowSeconds"),
        "aliveAfterWindow": alive,
