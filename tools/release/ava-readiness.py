...
def validate_model(model: dict) -> list[str]:
    ...
    if model.get("releaseDecision") == "HOMOLOGATED":
        if points != 100:
...
def parse_trx(path: Path) -> dict:
    ...
    counters = next((node.attrib for node in root.iter() if node.tag.endswith("Counters")), None)
    if counters is None:
    ...
    total = n("total")
    passed = n("passed")
    failed = n("failed") + n("error") + n("timeout") + n("aborted") + n("inconclusive") + n("passedButRunAborted") + n("warning")
    not_executed = n("notExecuted") + n("notRunnable") + n("disconnected") + n("pending") + n("inProgress")
    status = PASS if total > 0 and failed == 0 and not_executed == 0 and passed == total else FAIL
    return {"status": status, "total": total, "passed": passed, "failed": failed, "notExecuted": not_executed, "sha256": sha256_file(path), "file": path.name}
...
def artifact_build_evidence(directory: Path, outcome: str, required_filename: str) -> tuple[dict, list[dict]]:
    ...
    if directory.is_dir():
        for path in sorted(directory.rglob("*")):
            if path.is_file():
                files.append({"file": path.relative_to(directory).as_posix(), "sizeBytes": path.stat().st_size, "sha256": sha256_file(path)})
    ...
    return ({"status": status, "outcome": outcome, "artifactCount": len(files), "requiredFile": required_filename, "requiredFilePresent": required_present}, files)
...
def parse_soak(path: Path, expected_engine_hashes: set[str]) -> dict:
    ...
    status = PASS if payload.get("pass") is True and requested > 0 and completed == requested and not failures and hash_bound else FAIL
    return {"status": status, "reportSha256": report_hash, "engineHostSha256": engine_hash, "engineHashBoundToPublishedCandidate": hash_bound, "requestedIterations": requested, "completedIterations": completed, "restartCount": payload.get("restartCount"), "durationMilliseconds": payload.get("durationMilliseconds"), "latencyMilliseconds": payload.get("latencyMilliseconds"), "resources": payload.get("resources"), "failures": failures}


def parse_recovery(path: Path, expected_engine_hashes: set[str]) -> dict:
    if not path.is_file():
        return {"status": BLOCKED, "reason": "EngineHost recovery report missing", "path": str(path)}
    try:
        payload = load_json(path)
    except (OSError, json.JSONDecodeError) as exc:
        return {"status": INCONCLUSIVE, "reason": f"recovery report parse failed: {exc}", "path": str(path)}
    report_hash = sha256_file(path)
    engine_hash = str(payload.get("engineHostSha256") or "").lower()
    failures = payload.get("failures") or []
    hash_bound = bool(engine_hash) and engine_hash in {value.lower() for value in expected_engine_hashes}
    required = (
        ...
        and payload.get("firstPingPassed") is True
        and payload.get("crashObserved") is True
        and payload.get("failClosedAfterCrash") is True
        and payload.get("restartPassed") is True
        and payload.get("secondPingPassed") is True
    ...
    return {
        ...
        "firstProcessId": payload.get("firstProcessId"),
        "secondProcessId": payload.get("secondProcessId"),
        "firstPingPassed": payload.get("firstPingPassed") is True,
        "crashObserved": payload.get("crashObserved") is True,
        "failClosedAfterCrash": payload.get("failClosedAfterCrash") is True,
        "restartPassed": payload.get("restartPassed") is True,
        "secondPingPassed": payload.get("secondPingPassed") is True,
...
def parse_desktop_startup(path: Path, expected_desktop_hashes: set[str]) -> dict:
    if not path.is_file():
        return {"status": BLOCKED, "reason": "Desktop startup report missing", "path": str(path)}
    try:
        payload = load_json(path)
    except (OSError, json.JSONDecodeError) as exc:
        return {"status": INCONCLUSIVE, "reason": f"Desktop startup report parse failed: {exc}", "path": str(path)}
    report_hash = sha256_file(path)
    desktop_hash = str(payload.get("desktopSha256") or "").lower()
    failures = payload.get("failures") or []
    alive = payload.get("aliveAfterWindow") is True
    cleanup = payload.get("cleanupVerified") is True
    hash_bound = bool(desktop_hash) and desktop_hash in {value.lower() for value in expected_desktop_hashes}
    status = PASS if payload.get("pass") is True and alive and cleanup and not failures and hash_bound else FAIL
    return {"status": status, "reportSha256": report_hash, "desktopSha256": desktop_hash, "desktopHashBoundToPublishedCandidate": hash_bound, "aliveWindowSeconds": payload.get("aliveWindowSeconds"), "aliveAfterWindow": alive, "cleanupVerified": cleanup, "processId": payload.get("processId"), "exitCode": payload.get("exitCode"), "failures": failures, "scope": payload.get("scope")}
...
def discover_desktop_surface(repo_root: Path) -> dict:
    ...
    for project in sorted(src.rglob("*.csproj")):
        ...
        if normalized.endswith("aevrix.core.csproj") or normalized.endswith("aevrix.enginehost.csproj"):
            continue
        try:
            lowered = project.read_text(encoding="utf-8").lower()
        except OSError:
            continue
        if any(signal in lowered for signal in ("<outputtype>winexe</outputtype>", "<usewinui>true</usewinui>", "microsoft.windowsappsdk")):
            candidates.append(project.relative_to(repo_root).as_posix())
    if not candidates:
        return {"status": BLOCKED, "reason": "No physical Desktop/WinUI project was detected; end-to-end product smoke cannot run.", "projects": []}
    return {"status": PASS, "reason": "Physical Desktop/WinUI project detected. End-to-end execution remains a separate mandatory gate.", "projects": candidates}
...
def load_external_evidence(path: Path | None, source_commit: str) -> tuple[dict, list[str]]:
    ...
    accepted: dict = {}
    for gate_id, evidence in (payload.get("gates") or {}).items():
...
def candidate_command(args: argparse.Namespace) -> int:
    ...
    enginehost_build, enginehost_files = artifact_build_evidence(Path(args.enginehost_dir), args.build_outcome, "AEVRIX.EngineHost.exe")
    desktop_build, desktop_files = artifact_build_evidence(Path(args.desktop_dir), args.desktop_build_outcome, "AEVRIX.Desktop.exe")
    engine_hashes = {entry["sha256"] for entry in enginehost_files}
    desktop_hashes = {entry["sha256"] for entry in desktop_files}
    soak = parse_soak(Path(args.soak_json), engine_hashes)
    recovery = parse_recovery(Path(args.recovery_json), engine_hashes)
    desktop_startup = parse_desktop_startup(Path(args.desktop_startup_json), desktop_hashes)
    ...
    exact_gates.update(external_gates)
    ...
    payload = {
        ...
        "runner": {"os": args.runner_os, "image": args.runner_image, "architecture": args.runner_arch, "dotnetSdk": args.dotnet_sdk},
    ...
    (out_dir / "candidate-evidence.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
...
def main() -> int:
    ...
    candidate.add_argument("--desktop-startup-json", required=True)
    ...
    candidate.add_argument("--recovery-json", required=True)
