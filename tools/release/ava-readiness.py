...
def parse_trx(path: Path) -> dict:
    ...
    total = n("total")
    passed = n("passed")
    failed = n("failed") + n("error") + n("timeout") + n("aborted")
    not_executed = n("notExecuted") + n("notRunnable") + n("disconnected") + n("pending")
...
def discover_desktop_surface(repo_root: Path) -> dict:
    ...
    return {
        "status": PARTIAL,
        "reason": "Desktop product project exists, but existence alone is not end-to-end runtime evidence.",
        "projects": candidates,
    }
...
def candidate_command(args: argparse.Namespace) -> int:
    ...
    desktop = discover_desktop_surface(repo_root)

    enginehost_dir = Path(args.enginehost_dir)
    enginehost_files = []
    if enginehost_dir.is_dir():
        for path in sorted(enginehost_dir.rglob("*")):
            if path.is_file():
                enginehost_files.append(
                    {
                        "file": path.relative_to(enginehost_dir).as_posix(),
    ...
    exact_gates = {
        "enginehost-build": {
            "status": build_status,
            "artifactCount": len(enginehost_files),
        },
        "core-tests": core,
