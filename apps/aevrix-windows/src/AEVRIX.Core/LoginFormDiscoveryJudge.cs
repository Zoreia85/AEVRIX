...
/// <summary>
/// Core-side judge for browser-collected DOM metadata. It never consumes input values and only produces a
/// LoginRecipe when username, password and submit controls can be selected uniquely inside one form boundary.
/// </summary>
public static class LoginFormDiscoveryJudge
{
        ...
        LoginFormSnapshot snapshot)
    {
        ...
        {
            if (!ValidateElement(element) || !selectors.Add(element.Selector))
            {
        ...
        {
            return LoginFormDiscoveryResult.Ambiguous("multiple_password_fields", passwordFields);
        }

        var passwordField = passwordFields[0];
        var formKey = passwordField.FormKey;

        var userSelection = SelectUniqueTop(active
            .Where(item => string.Equals(item.FormKey, formKey, StringComparison.Ordinal))
            .Where(item => item.DocumentOrder < passwordField.DocumentOrder)
        ...
        var submitSelection = SelectUniqueTop(active
            .Where(item => string.Equals(item.FormKey, formKey, StringComparison.Ordinal))
    ...
    {
        var autoComplete = element.AutoComplete?.Trim() ?? string.Empty;
        ...
        if (ContainsAny(descriptor, "username", "user", "login", "account", "usuário", "usuario"))
        {
            return 600;
        }
    ...
    {
        var score = string.Equals(element.InputType, "submit", StringComparison.OrdinalIgnoreCase)
            ? 1000
        ...
        if (ContainsAny(
            Descriptor(element),
            "sign in", "signin", "log in", "login", "continue", "next", "entrar", "acessar"))
        {
            score += 100;
        }
        return score;
