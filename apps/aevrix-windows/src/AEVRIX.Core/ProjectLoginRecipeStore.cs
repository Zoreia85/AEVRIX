...
/// <summary>
/// Persists non-secret login-form recipes inside the owning project. A recipe contains selectors and
/// session markers only; usernames/passwords remain exclusively in ProjectCredentialVault.
/// Persisted entries are revalidated against the current project target and browser policy on every read.
/// </summary>
public sealed class ProjectLoginRecipeStore
{
    ...
    private static void ValidateRegistryForProject(RegistryDocument document, ProjectEnvelope envelope)
    {
        ...
        foreach (var item in document.Entries)
        {
            ValidateRecipeAgainstProject(envelope, policy, item.Recipe);
        }
    }
    ...
    private static void ValidateRecipeAgainstProject(
    ...
    private static void ValidateSelector(string selector, string parameterName)
    {
        ...
        if (selector.Length > MaxSelectorLength || selector.Any(char.IsControl))
        {
    ...
    private static void ValidateMarkers(IReadOnlyList<string> markers, string parameterName)
    {
        ...
        if (markers.Count > MaxMarkersPerCollection)
        {
