...
public sealed class ProjectCurrentPageLoginServiceTests
{
    ...
    public async Task ExecuteAsync_CurrentPageAutomaticallyResolvesRecipeAndSubmits()
    {
        ...
        var result = await fixture.Service.ExecuteAsync(
            new(project.Project.Id, true, true),
            adapter);
    ...
    public async Task ExecuteAsync_AmbiguousCredentialsReturnsSelectionWithoutReadingSecret()
    {
        ...
        var firstSecret = new string('a', 16);
        var secondSecret = new string('b', 16);
        await fixture.Vault.AddAsync(project.Project.Id, "A", recipe.LoginUri, "fixture-a@example.com", firstSecret, makeDefaultForLoginUri: false);
        await fixture.Vault.AddAsync(project.Project.Id, "B", recipe.LoginUri, "fixture-b@example.com", secondSecret, makeDefaultForLoginUri: false);
        fixture.SecretStore.ResetReadCount();
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        {
            var project = CaptureProject.CreateWeb("Current Page Login", "target-web", new Uri("https://example.com/"));
        ...
        public async Task AddRecipeAndCredentialAsync(Guid projectId)
        {
            ...
            var secretValue = new string('s', 20);
            await Vault.AddAsync(projectId, "Primary", recipe.LoginUri, "fixture-user@example.com", secretValue);
        }
