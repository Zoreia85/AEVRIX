...
public sealed class LoginFormSnapshotParserTests
{
    ...
    {
        var snapshot = LoginFormSnapshotParser.Parse(
    ...
    public void Parse_UnknownValuePropertyIsRejected()
    {
        var payload = ValidPayload().Replace(
            "\"name\":\"account\"",
            "\"name\":\"account\",\"value\":\"forbidden\"",
            StringComparison.Ordinal);

        Assert.Throws<InvalidDataException>(() => LoginFormSnapshotParser.Parse(
            new Uri("https://example.com/login"),
            payload,
            DateTimeOffset.UtcNow));
    }
    ...
    public void Parse_UnknownRawHtmlPropertyIsRejected()
    {
        var payload = ValidPayload().Replace(
            "\"name\":\"account\"",
            "\"name\":\"account\",\"outerHtml\":\"forbidden\"",
            StringComparison.Ordinal);
