...
public sealed class AiProviderInvocationCoordinatorTests
{
    ...
    public void StartedMeteredInvocation_CannotReleaseReservationAndRiskDuplicateSpend()
    {
        ...
        Assert.IsTrue(coordinator.Reserve(Estimate("paid-1", 900)).Allowed);
        ...
        Assert.Throws<InvalidOperationException>(() => coordinator.CancelBeforeInvocation(ProjectId, "paid-1"));
    ...
    private static AiProviderCallEstimate Estimate(string requestId, long cost) =>
        new(ProjectId, requestId, "remote-premium", AiProviderLocation.Remote, AiProviderBillingMode.ExternalMetered,
            "USD", cost, 100, 100, 500);
