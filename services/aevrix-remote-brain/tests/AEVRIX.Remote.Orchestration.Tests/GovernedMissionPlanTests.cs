...
public sealed class GovernedMissionPlanTests
{
    ...
    {
        var plan = BuildPlan();
        var request = new MissionPlanningRequest(plan, Scope(plan), [Authority("task-a")], []);
    ...
    public void ControlPlaneEnforcesCanonicalProgressionAndDigestChain()
    {
        ...
        var current = MissionControlSnapshot.CreateInitial(governed, t0);
    ...
    private static MissionTaskAuthority Authority(string taskId) =>
        new(taskId, MissionRuntimeAuthority.ReadProjectWorkspace, [], 268_435_456, TimeSpan.FromMinutes(10));
}
