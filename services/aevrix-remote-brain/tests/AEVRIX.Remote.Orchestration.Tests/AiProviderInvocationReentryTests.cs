...
public sealed class AiProviderInvocationReentryTests
{
    ...
    public void StartedInvocation_CannotReserveSameRequestForSecondProviderCall()
    {
        ...
        var policy = new AiProjectBudgetPolicy(projectId, AiProviderBudgetProfile.Balanced, "USD", 1_000, 1, 1_000, 1_000);
        var budgets = new AiProviderBudgetManager([policy]);
        var coordinator = new AiProviderInvocationCoordinator(budgets);
        var estimate = new AiProviderCallEstimate(projectId, "paid-reentry", "remote-premium",
            AiProviderLocation.Remote, AiProviderBillingMode.ExternalMetered, "USD", 500, 100, 100, 500);
