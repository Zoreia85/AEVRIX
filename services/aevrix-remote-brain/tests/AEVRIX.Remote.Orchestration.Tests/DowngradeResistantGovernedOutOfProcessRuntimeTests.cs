...
public sealed class DowngradeResistantGovernedOutOfProcessRuntimeTests
{
    ...
    public async Task ExecuteAsync_MissingFloorBlocksBeforeBackendExecution()
    {
        ...
        var runtime = Runtime(backend, backendEpoch: 5, policyEpoch: 7, floor: null);
        using var workspace = new TempDirectory();

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            runtime.ExecuteAsync(new OutOfProcessExecutionRequest([], workspace.Path)));
    ...
    {
        var backend = new CountingBackend();
        var runtime = Runtime(backend, backendEpoch: 4, policyEpoch: 7, floor: new ExecutionVersionFloor(5, 7));
        using var workspace = new TempDirectory();
    ...
    {
        var backend = new CountingBackend();
        var runtime = Runtime(backend, backendEpoch: 5, policyEpoch: 6, floor: new ExecutionVersionFloor(5, 7));
        using var workspace = new TempDirectory();
    ...
    {
        var backend = new CountingBackend();
        var runtime = Runtime(backend, backendEpoch: 5, policyEpoch: 7, floor: new ExecutionVersionFloor(5, 7));
        using var workspace = new TempDirectory();
    ...
    public void Constructor_RejectsUnversionedBackendAndPolicyEpoch()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new VersionedIsolationBackendRegistration(backend, 0).Validate());
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new DowngradeResistantGovernedOutOfProcessRuntime(
                [new VersionedIsolationBackendRegistration(backend, 1)],
                RestrictedAuthority(),
                0,
                gate));
    }

    private static DowngradeResistantGovernedOutOfProcessRuntime Runtime(
        CountingBackend backend,
        ulong backendEpoch,
        ulong policyEpoch,
        ExecutionVersionFloor? floor)
    {
        var gate = new MonotonicExecutionVersionGate(new InMemoryFloorAnchor(floor), "runtime:test");
        return new DowngradeResistantGovernedOutOfProcessRuntime(
            [new VersionedIsolationBackendRegistration(backend, backendEpoch)],
            RestrictedAuthority(),
            policyEpoch,
            gate);
    }
    ...
    private sealed class InMemoryFloorAnchor(ExecutionVersionFloor? initial) : IExecutionVersionFloorAnchor
    {
        ...
        public Task<ExecutionVersionFloor?> LoadAsync(
            string scopeId,
            CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (effective != expectedPrevious)
                throw new InvalidOperationException("stale compare-and-swap");
            _current = next;
