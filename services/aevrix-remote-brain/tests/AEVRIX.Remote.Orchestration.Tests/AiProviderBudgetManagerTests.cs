...
public sealed class AiProviderBudgetManagerTests
{
    ...
    public void Budgets_AreStrictlyIsolatedByProject()
    {
        ...
        Assert.IsTrue(manager.Reserve(Estimate(ProjectA, "a-1", "remote", AiProviderLocation.Remote, cost: 900, input: 80, output: 70)).Allowed);
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => Policy(ProjectA, AiProviderBudgetProfile.Custom, spend: -1).Validate());
    ...
    public void Contracts_DoNotExposeCredentialOrAuthorizationMaterial()
    {
        ...
        {
            typeof(AiProjectBudgetPolicy), typeof(AiProviderCallEstimate), typeof(AiProviderBudgetReservation),
            typeof(AiProviderUsageReceipt), typeof(AiProjectBudgetSnapshot)
        };
    ...
    private static AiProjectBudgetPolicy Policy(Guid projectId, AiProviderBudgetProfile profile, long spend = 10_000,
        int calls = 10, long input = 10_000, long output = 10_000) =>
        new(projectId, profile, "USD", spend, calls, input, output);

    private static AiProviderCallEstimate Estimate(Guid projectId, string requestId, string providerId,
        AiProviderLocation location, long cost = 0, long input = 10, long output = 10) =>
        new(projectId, requestId, providerId, location,
            location == AiProviderLocation.Remote ? AiProviderBillingMode.ExternalMetered : AiProviderBillingMode.None,
            "USD", cost, input, output, 100);
