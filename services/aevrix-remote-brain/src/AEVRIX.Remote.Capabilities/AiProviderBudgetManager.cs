    ...
    long ActualOutputTokens)
{
    public AiProviderUsageReceipt Validate()
    {
        ...
        if (ActualCostMicros < 0 || ActualInputTokens < 0 || ActualOutputTokens < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(ActualCostMicros), "Usage values cannot be negative.");
        }
...
public sealed class AiProviderBudgetManager
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        var validated = policies.Select(policy => (policy ?? throw new ArgumentException("Policies cannot contain null entries.", nameof(policies))).Validate()).ToArray();
    ...
    public AiBudgetReservationDecision Reserve(AiProviderCallEstimate estimate)
    {
        ...
        lock (_sync)
        {
            ...
            if (state.Completed.TryGetValue(estimate.RequestId, out _))
            {
            ...
            {
                if (ReservationMatchesEstimate(existing, estimate))
                {
                    return AiBudgetReservationDecision.Permit(existing, Snapshot(state));
                }

                return AiBudgetReservationDecision.Deny(AiBudgetDenialReason.RequestIdConflict, Snapshot(state));
            }
