    ...
    OutOfProcessFilesystemPolicy Filesystem)
{
    ...
    public bool RequiresUnavailableIsolationBackend => Network.RequiresIsolation || Filesystem.RequiresIsolation;
...
/// <summary>Replaceable execution backend for one authority profile. Filesystem-restricted authority requires independent external-write and external-read proof.</summary>
public interface IOutOfProcessIsolationBackend
{
    string BackendId { get; }
    int Priority { get; }
    bool CanEnforce(OutOfProcessAuthorityPolicy authority);
    Task<OutOfProcessExecutionResult> ExecuteAsync(OutOfProcessAuthorityPolicy authority, OutOfProcessExecutionRequest request, CancellationToken cancellationToken = default);
    IsolationAuthorityAttestation AttestAuthority(OutOfProcessAuthorityPolicy authority, OutOfProcessExecutionResult execution);
}
...
public sealed class LocalUnrestrictedOutOfProcessBackend : IOutOfProcessIsolationBackend
{
    ...
    public LocalUnrestrictedOutOfProcessBackend(PinnedOutOfProcessRuntime runtime) => _runtime = runtime ?? throw new ArgumentNullException(nameof(runtime));
    public string BackendId => "local-unrestricted";
    public int Priority => 0;
    public bool CanEnforce(OutOfProcessAuthorityPolicy authority)
    {
        ArgumentNullException.ThrowIfNull(authority); authority.Validate(); return !authority.RequiresUnavailableIsolationBackend;
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(authority); ArgumentNullException.ThrowIfNull(request); authority.Validate(); request.Validate();
        if (!CanEnforce(authority)) throw new InvalidOperationException("The unrestricted local-process backend cannot enforce the requested isolation authority.");
        return _runtime.ExecuteAsync(request, cancellationToken);
    }
    public IsolationAuthorityAttestation AttestAuthority(OutOfProcessAuthorityPolicy authority, OutOfProcessExecutionResult execution)
    {
        ArgumentNullException.ThrowIfNull(authority); ArgumentNullException.ThrowIfNull(execution); return new(BackendId, authority.ComputeFingerprint());
    }
}
...
public sealed class GovernedOutOfProcessRuntime
{
    ...
    public GovernedOutOfProcessRuntime(PinnedOutOfProcessRuntime runtime, OutOfProcessAuthorityPolicy authority)
        : this([new LocalUnrestrictedOutOfProcessBackend(runtime)], authority) { }

    public GovernedOutOfProcessRuntime(IEnumerable<IOutOfProcessIsolationBackend> backends, OutOfProcessAuthorityPolicy authority)
    {
        ArgumentNullException.ThrowIfNull(backends);
        _authority = (authority ?? throw new ArgumentNullException(nameof(authority))).Validate();
        var list = backends.ToArray();
        if (list.Length is < 1 or > 16) throw new ArgumentException("Between one and sixteen isolation backends must be registered.", nameof(backends));
        foreach (var backend in list)
            if (backend is null || !IsSafeBackendId(backend.BackendId) || backend.Priority is < -1_000 or > 1_000)
        ...
        _backends = list.OrderByDescending(item => item.Priority).ThenBy(item => item.BackendId, StringComparer.Ordinal).ToArray();
    }
    ...
    public OutOfProcessAuthorityDecision EvaluateAuthority()
    {
        ...
        if (selected is not null)
            return new(_authority.Network.Scope, _authority.Filesystem.Scope, true,
                selected is LocalUnrestrictedOutOfProcessBackend ? "AuthorizedUnrestrictedLocalProcess" : "AuthorizedByIsolationBackend", selected.BackendId);
        if (_authority.Network.RequiresIsolation && _authority.Filesystem.RequiresIsolation)
            return new(_authority.Network.Scope, _authority.Filesystem.Scope, false, "NetworkAndFilesystemIsolationBackendUnavailable");
        if (_authority.Network.RequiresIsolation)
            return new(_authority.Network.Scope, _authority.Filesystem.Scope, false, "NetworkIsolationBackendUnavailable");
        if (_authority.Filesystem.RequiresIsolation)
            return new(_authority.Network.Scope, _authority.Filesystem.Scope, false, "FilesystemIsolationBackendUnavailable");
        return new(_authority.Network.Scope, _authority.Filesystem.Scope, false, "NoCompatibleExecutionBackend");
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(request); request.Validate(); cancellationToken.ThrowIfCancellationRequested();
        var decision = EvaluateAuthority();
        if (!decision.LaunchAuthorized || decision.SelectedBackendId is null)
            throw new InvalidOperationException($"Pinned process launch denied by execution authority gate ({decision.DecisionCode}). Network={decision.NetworkScope}; Filesystem={decision.FilesystemScope}. No registered backend can prove the complete requested isolation boundary.");
        var backend = _backends.Single(item => string.Equals(item.BackendId, decision.SelectedBackendId, StringComparison.OrdinalIgnoreCase));
    ...
    {
        ArgumentNullException.ThrowIfNull(backend); ArgumentNullException.ThrowIfNull(result);
        var attestation = result.Attestation ?? throw new InvalidDataException("Execution backend returned no execution attestation.");
        var binding = (backend.AttestAuthority(_authority, result) ?? throw new InvalidDataException("Execution backend returned no authority binding.")).Validate();
        ...
        finally { CryptographicOperations.ZeroMemory(expectedBytes); CryptographicOperations.ZeroMemory(actualBytes); }
    ...
    internal static bool IsSafeBackendId(string value) => !string.IsNullOrWhiteSpace(value) && value.Length is >= 3 and <= 96 && value.All(ch => char.IsAsciiLetterOrDigit(ch) || ch is '-' or '_' or '.');
}
