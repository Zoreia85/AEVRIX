...
public sealed class EvidenceFusionModelProvider : IAevrixModelProvider
{
    ...
    public Task<ModelAnalysisCandidate> AnalyzeAsync(AnalysisTask task, CancellationToken cancellationToken = default)
    {
        ...
        if (!task.Context.TryGetValue(ClaimKeyContext, out var claimKey)
        ...
        var observations = _bus.Snapshot(task.ProjectId, task.TargetId)
            .Where(item => string.Equals(item.ClaimKey, claimKey, StringComparison.OrdinalIgnoreCase)
                && allowed.Contains(item.EvidenceId))
            .ToArray();
        ...
        var statement = fused.State switch
        {
            ...
            EvidenceFusionState.Contested => $"Contested evidence for {claimKey}: "
                + string.Join(" | ", fused.Alternatives.Select(item => item.RepresentativeValue)),
