...
public sealed record MissionKnowledgeItem(
    string ClaimKey,
    EvidenceFusionState FusionState,
    CandidateKnowledge Knowledge);

public sealed record MissionKnowledgeResult(
    MissionExecutionResult Mission,
    IReadOnlyList<MissionKnowledgeItem> KnowledgeItems);
...
public sealed class MissionEvidenceJudgePipeline
{
    ...
    public async Task<MissionKnowledgeResult> ExecuteAsync(
        MissionKnowledgeRequest request,
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var claimKey in request.ClaimKeys.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            var task = new AnalysisTask(
                BuildTaskId(mission.MissionId, claimKey),
                mission.ProjectId,
                mission.TargetId,
                $"Judge fused evidence claim '{claimKey}'.",
                fused.EvidenceIds,
            ...
            {
                if (request.ProofRecords is not null && request.ExpectedProofHead is not null)
                {
