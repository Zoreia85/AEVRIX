    ...
    IReadOnlyDictionary<string, string> Context)
{
    ...
    private static bool IsSafeId(string value, int min, int max) =>
        !string.IsNullOrWhiteSpace(value) && value.Length >= min && value.Length <= max
...
{
    public bool EligibleForTrustedPromotion => EvidenceIntegrityPassed && EvidenceSupportsStatement
        && IndependentValidationPassed && CounterexampleReviewPassed && ValidatedEvidenceIds.Count > 0;
    ...
    ModelRiskLevel MaximumSingleProviderRisk = ModelRiskLevel.Low)
{
    ...
    {
        if (PrimaryAcceptConfidence < 0.5 || PrimaryAcceptConfidence > 1
            || EscalationConfidence < 0 || EscalationConfidence > PrimaryAcceptConfidence)
...
public sealed class OrchestratorJudge
{
    ...
    public async Task<CandidateKnowledge> AnalyzeToCandidateAsync(AnalysisTask task, CancellationToken cancellationToken = default)
    {
        ...
        var mustEscalate = primary.Confidence < _policy.PrimaryAcceptConfidence || primary.Risk > _policy.MaximumSingleProviderRisk;
        ...
        var candidate = new CandidateKnowledge(
            ...
            trace, selected.Assumptions.Distinct(StringComparer.Ordinal).Take(256).ToArray(),
            selected.OpenQuestions.Distinct(StringComparer.Ordinal).Take(256).ToArray(), now, now);
        await _repository.StoreCandidateAsync(candidate, cancellationToken);
        return candidate;
    }

    public Task<CandidateKnowledge> ValidateAndPromoteAsync(string knowledgeId, CancellationToken cancellationToken = default) =>
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (validation.EligibleForTrustedPromotion)
        {
            ...
            if (trusted.TrustState != KnowledgeTrustState.Trusted)
                throw new InvalidDataException("Memory admission gate did not produce Trusted repository state.");
            return trusted;
        }

        var targetState = validation.EvidenceIntegrityPassed && validation.EvidenceSupportsStatement
            ? KnowledgeTrustState.Validated : KnowledgeTrustState.Rejected;
        await _repository.PromoteAsync(candidate.KnowledgeId, targetState, validation.ValidationRecordId, _time.GetUtcNow(), cancellationToken);
        return candidate with { TrustState = targetState, ValidationRecordId = validation.ValidationRecordId, UpdatedAt = _time.GetUtcNow() };
    }

    private static ModelAnalysisCandidate Consolidate(ModelAnalysisCandidate primary, ModelAnalysisCandidate secondary)
    {
        var sharedEvidence = primary.EvidenceIds.Intersect(secondary.EvidenceIds, StringComparer.Ordinal).Distinct(StringComparer.Ordinal).ToArray();
        var allEvidence = primary.EvidenceIds.Concat(secondary.EvidenceIds).Distinct(StringComparer.Ordinal).ToArray();
        ...
        var statement = string.Equals(primary.Statement.Trim(), secondary.Statement.Trim(), StringComparison.Ordinal)
            ? primary.Statement.Trim() : $"Primary candidate: {primary.Statement.Trim()}\nSecondary candidate: {secondary.Statement.Trim()}";
        return new ModelAnalysisCandidate(
            ...
            statement, confidence, (ModelRiskLevel)Math.Max((int)primary.Risk, (int)secondary.Risk), allEvidence,
    ...
    private static void EnsureEvidenceSubset(AnalysisTask task, ModelAnalysisCandidate candidate)
    {
        ...
        if (unknown.Length > 0) throw new InvalidDataException("Model candidate referenced evidence outside the governed task context.");
    }
    ...
    private static string BuildKnowledgeId(AnalysisTask task, ModelAnalysisCandidate candidate)
    {
        ...
        {
            task.ProjectId.ToString("D"), task.TargetId, task.TaskId, candidate.Statement.Trim(),
