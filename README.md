...
The public-repository presentation, Windows installer visual system and screenshot provenance pipeline are specified in [`docs/product/GITHUB_SHOWCASE_INSTALLER_AND_VISUAL_ASSET_PIPELINE.md`](docs/product/GITHUB_SHOWCASE_INSTALLER_AND_VISUAL_ASSET_PIPELINE.md). The target README experience places real AEVRIX software captures near the top of the repository and binds production screenshots to the exact build/commit they represent.

Concept visuals are not evidence of release readiness. Production screenshots will be linked only to builds that can be reproduced and tested; concepts must remain explicitly labeled until replaced by real captures.

## Extensible capability strategy

AEVRIX can evaluate third-party agent frameworks, retrieval systems, coding harnesses and quantum/hybrid solvers as replaceable specialist capabilities without surrendering the native Mission Director, evidence boundary or Orchestrator/Judge. Admission rules and the quantum/hybrid benchmark gate are documented in [`docs/architecture/EXTERNAL_AGENT_AND_QUANTUM_CAPABILITY_STRATEGY.md`](docs/architecture/EXTERNAL_AGENT_AND_QUANTUM_CAPABILITY_STRATEGY.md).
